...
s = requests.session()
my_info = json.loads(s.get('http://api.bilibili.com/x/space/myinfo',allow_redirects=False,headers=header).text)
...
if 'video/av' in url:
    av = json.loads(s.get('https://api.bilibili.com/x/web-interface/archive/stat?aid='+url,allow_redirects=False,headers=header).text)
    url = av['data']['bvid']
video_id = re.findall("[\w.]*[\w:\-\+\%]",url)[3]
vid = json.loads(s.get('https://api.bilibili.com/x/web-interface/view?bvid='+video_id,allow_redirects=False,headers=header).text)
...
video_info = json.loads(s.get('https://api.bilibili.com/x/player/playurl?bvid='+video_id+'&cid='+str(video)+'&qn=80&otype=json',allow_redirects=False,headers=header).text)
